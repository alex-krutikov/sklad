...
//==============================================================================
  ...
  if( !query_str_color_field.isEmpty() )
    str += query_str_color_field + ",";
  for( i=0; i<cc; i++ )
  { if( header_show_flag[i] )
    {  str += query_fields[i] + ",";
       ...
       { str2 = " ORDER BY " + query_fields[i];
         if( horizontalHeader()->sortIndicatorOrder() == Qt::DescendingOrder )
         { str2 += " DESC ";
         } else
         { str2 += " ASC ";
         }
    ...
    { switch( v[k] )
      { case( QVariant::Int ):
        ...
        case( QVariant::ByteArray ):
          ...
          newItem->setFlags(Qt::ItemIsSelectable|Qt::ItemIsEnabled);
          if( !query.value(k+qq).isNull() )
          { newItem->setText( query.value(k+qq).toString().replace("\n"," ") );
          }
          a = alignment.value(k);
        ...
        case( QVariant::Date ):
          newItem = new QTableWidgetItem;
          newItem->setFlags(Qt::ItemIsSelectable | Qt::ItemIsEnabled);
          date = QDate::fromString( query.value(k+qq).toString(), "yyyy-MM-dd" );
          if( !query.value(k+qq).isNull() )
...
void SqlTable::my2(int i)
{
  ...
  if(     ( hw->sortIndicatorSection() == i )
      &&  ( hw->isSortIndicatorShown() ) )
...
void KomplTable::refresh( int id )
{
  ...
  QSqlQuery query;
  query.prepare( " SELECT kompl.id, kompl.color, kompl.type, typename, kompl.name, kompl.need, "
...
void KomplTable::add_zamena()
{
  ...
  if( dialog.exec() == QDialog::Accepted ) mainwindow->on_action_Pereraschet_triggered();
}
...
void KomplTable::status()
{
  ...
  else if( item_selected == item2 ) j=2;
  else if( item_selected == item3 ) j=0;
  else return;
...
void Defichit2::refresh()
{
  ...
  query.prepare(
    ...
    " FROM ( "
    "   SELECT kompl.type,typename,zamena.name,SUM(kompl.need-kompl.snato) AS treb, "
...
void Defichit2::on_pb_print_clicked()
{
  ...
  { if( query.value(0).toInt() != k )
    { k = query.value(0).toInt();
       str += "<TR>";
       str += " <TD style=\"font-family:Tahoma\" bgcolor=lightgray colspan=4 > <B>" + query.value(1).toString() + "</B> </TD>";
       str += "</TR>";
    }
    str += "<TR>";
...
void Defichit2::on_tw_cellDoubleClicked(int row, int col)
{
 ...
 Defichit2RenameDialog dialog( this, tw->item( row, 0 )->text(),
...
Defichit2RenameDialog::Defichit2RenameDialog( QWidget *parent,
         const QString &name,  int type, const QString &kompl )
  : QDialog( parent )
{
  setupUi( this );
...
void Defichit2RenameDialog::on_le_filtr_textChanged(const QString & str_arg)
{
  ...
  query.prepare( " SELECT DISTINCT t1.name,t2.s1, t3.s2 FROM "
                 "           ( SELECT name,type FROM prihod UNION SELECT name,type FROM zakupki )  AS t1 "
                 " LEFT JOIN ( SELECT name,sum(ostatok) AS s1 FROM prihod GROUP BY name )      as t2 ON t1.name = t2.name "
                 " LEFT JOIN ( SELECT name,sum(n-polucheno) AS s2 FROM zakupki GROUP BY name ) as t3 ON t1.name = t3.name "
...
void Defichit2RenameDialog::on_pb_rename_clicked()
{
  ...
  query_str =
         " UPDATE kompl,zamena  SET kompl.name=?, zamena.name=? "
         " WHERE kompl.sostav IN( " + kompl + " ) "
         "       AND kompl.name=? AND zamena.name=? AND zamena.kompl=kompl.id ";
  query.prepare( query_str );
  query.addBindValue( str   );
...
static inline void map_insert(  QMap<QString,int> &map, const QString &str, const QString &str_type, int i )
{
  QString s = str_type+";"+str;
  map.insert( s, map.value(s,0) + i );
}

DefichitPriceDialog::DefichitPriceDialog( QWidget *parent, const QString &items_str )
  : QDialog( parent )
{
  setupUi(this);
  ...
  while( query.next() )
  { ost_map.insert( query.value(0).toString()+";"+query.value(1).toString(),
                    query.value(2).toInt() );
  }
  ...
  while( query.next() )
  { price_est_map.insert( query.value(0).toString(), query.value(1).toDouble() );
  }
  ...
  { query.prepare(
      " SELECT typename,kompl.name,zamena.name,kompl.need "
      " FROM kompl "
      " LEFT JOIN zamena ON zamena.kompl=kompl.id "
      " LEFT JOIN types ON kompl.type=types.id "
      " WHERE sostav = ? "
  ...
  { QString s = tw->item(i,0)->text();
    ...
    { tw->setItem( i,2, new QTableWidgetItem( QString::number(price_est_map.value(s),'f',2 ) ) );
    } else
    { tw->setItem( i,2, new QTableWidgetItem( "" ) );
    }
    if( price_map.contains(s) )
    { tw->setItem( i,3, new QTableWidgetItem( QString::number(price_map.value(s),'f',2 ) ) );
    } else
  ...
  { QString p1 = tw->item(i,2)->text();
    ...
    double p=0;
    int    n=0;
    if(      !p1.isEmpty() ) p=p1.toDouble();
    else if( !p2.isEmpty() ) p=p2.toDouble();
