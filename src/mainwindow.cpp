...
MainWindow::MainWindow()
{
    ...
    if (!(permissions & USER_PERMISSION_USERS)) action_Users->setVisible(false);

    QShortcut *shortcut1 = new QShortcut(QKeySequence(Qt::Key_F8), this);
    connect(shortcut1, SIGNAL(activated()), this, SLOT(my_Delete()));
    QShortcut *shortcut2 = new QShortcut(QKeySequence(Qt::Key_Delete), this);
    connect(shortcut2, SIGNAL(activated()), this, SLOT(my_Delete()));


    connect(tw_prihodHist, SIGNAL(cellActivated(int, int)), this, SLOT(my1A()));
    connect(le_prihodFiltr, SIGNAL(textChanged(const QString &)), this,
            SLOT(my1(QString)));
    connect(pb_kompl_addZamena, SIGNAL(clicked()), tw_komplKomp,
            SLOT(add_zamena()));
    connect(pb_komplStatus, SIGNAL(clicked()), tw_komplKomp, SLOT(status()));
    connect(tabWidget, SIGNAL(currentChanged(int)), this, SLOT(my3(int)));
    connect(tw_zakupkiHist, SIGNAL(cellActivated(int, int)), this, SLOT(my4()));
    connect(le_skladFiltr, SIGNAL(textChanged(const QString &)), this,
            SLOT(my5()));
    connect(cb_sklad_type, SIGNAL(currentIndexChanged(int)), this, SLOT(my5()));
    connect(le_zakupkiFiltr, SIGNAL(textChanged(const QString &)), this,
            SLOT(my6()));
    connect(le_zakupkiFiltrSchet, SIGNAL(textChanged(const QString &)), this,
            SLOT(my6()));
    connect(cb_zakupkiFiltrPostavshik, SIGNAL(currentIndexChanged(int)), this,
            SLOT(my6()));
    connect(cb_sklad_type_zakupki, SIGNAL(currentIndexChanged(int)), this,
            SLOT(my6()));
    connect(le_rashodFiltr, SIGNAL(textChanged(const QString &)), this,
            SLOT(my7()));
    connect(tw_komplPrih, SIGNAL(cellActivated(int, int)), this,
            SLOT(on_pb_komplSnat_clicked()));
    connect(tw_komplKomp, SIGNAL(cellActivated(int, int)), this,
            SLOT(on_pb_komplSnat_clicked()));
    connect(projects_widget, SIGNAL(pereraschet_needed()), this,
            SLOT(pereraschet()));
    ...
    tw_sklad->add_query_field("Цена", 70,
                              "SUM(price*kolichestvo)/SUM(kolichestvo) ");
    ...
    tw_prihodHist->query_str_main
        = " FROM prihod "
          ...
          " LEFT JOIN proizvoditeli   ON prihod.proizvoditel = "
          "proizvoditeli.id "
          " LEFT JOIN postavshiki     ON zakupki.postavshik = postavshiki.id "
          " LEFT JOIN users AS users1 ON prihod.user  = users1.id "
          " LEFT JOIN users AS users2 ON prihod.user2 = users2.id ";
    tw_prihodHist->query_str_order_by = " ORDER BY prihod.id DESC ";
    ...
    tw_rashodHist->query_str_order_by = " ORDER BY rashod.id DESC ";
    ...
    tw_komplZad->add_query_field("Статус", 80, "sostav.status2",
                                 Qt::AlignCenter);
    tw_komplZad->add_query_field("Зак.", 40, "sostav.zakupka_status",
                                 Qt::AlignCenter);
    tw_komplZad->add_query_field("Пр.", 40, "sostav.prioritet",
                                 Qt::AlignCenter);
    ...
    tw_komplPrih->query_str_main
        = " FROM prihod "
          ...
          " LEFT JOIN users AS users1 ON prihod.user  = users1.id "
          " LEFT JOIN users AS users2 ON prihod.user2 = users2.id ";
    tw_komplPrih->query_str_order_by = " ORDER BY prihod.id DESC ";
    ...
    QSqlQuery query;
    query.prepare(
        "SELECT id, typename FROM types WHERE used = 1 ORDER BY typename ASC");
    if (!query.exec())
    {
    ...
    while (query.next())
    {
        cb_zakupkiFiltrPostavshik->addItem(query.value(1).toString(),
                                           query.value(0));
    }
...
void MainWindow::my_Delete()
{
    ...
    case (3): {
        ...
        query.prepare(
            " SELECT name,zakupka,kolichestvo FROM prihod WHERE id = ? ");
        query.addBindValue(i);
        if (!query.exec())
        {
            sql_error_message(query, this);
        ...
        if (mb.exec() == QMessageBox::Yes)
        {
            ...
            if (zakupka_id)
            {
                query.prepare(" UPDATE zakupki SET polucheno = polucheno - ? "
                              "WHERE id = ? ");
    ...
    case (4): {
        ...
        query.prepare(
            "CREATE TEMPORARY TABLE t1 (id INT UNIQUE) ENGINE = MEMORY");
        if (!query.exec())
        {
            sql_error_message(query, this);
            return;
        ...
        if (mb.exec() == QMessageBox::Yes)
        {
            foreach (z, v)
            {
                ...
                query.prepare(
                    " UPDATE zamena SET snato = snato - ? WHERE id = ? ");
                ...
                query.prepare(
                    " UPDATE prihod SET ostatok = ostatok + ? WHERE id = ? ");
                query.addBindValue(n);
    ...
    case (5): {
        if (tw_komplZad->hasFocus()) // Удаление "На разборку"
        {
            ...
            QMessageBox mb(
                app_header, str, QMessageBox::Warning, QMessageBox::Yes,
            ...
            {
                query.prepare(
                    " DELETE sostav,kompl,zamena FROM sostav,kompl,zamena "
                    " WHERE sostav.id = ? "
        ...
        } else if (tw_komplKomp->hasFocus()) // Удаление "Замена"
        {
            ...
            if (tw_komplKomp->item(i, 0)->data(Qt::UserRole + 3) == 2)
            {
                ...
                QMessageBox mb(app_header, str, QMessageBox::Warning,
                               QMessageBox::Yes,
                               QMessageBox::No | QMessageBox::Default
                                   | QMessageBox::Escape,
                               QMessageBox::NoButton);
...
void MainWindow::on_action_Pereraschet_triggered()
{
    ...
    if (!query.exec(
            "  LOCK TABLES kompl WRITE, types WRITE, prihod WRITE, "
    ...
    //======
    query.prepare(" UPDATE kompl LEFT JOIN types ON kompl.type=types.id "
    ...
    //===================================
    query.prepare(" UPDATE kompl, "
                  "   ( SELECT kompl, SUM( snato ) AS s1 FROM zamena GROUP BY "
                  "kompl ) AS t1 "
    ...
    static const char query_text[]
        = "DROP TEMPORARY TABLE IF EXISTS t1;"
          //-------
          "CREATE  TEMPORARY TABLE t1 (name char(30) UNIQUE, s1 int) ENGINE = "
          "MEMORY "
          ...
          //-------
          "CREATE TEMPORARY TABLE t2 (kompl int UNIQUE, name char(30), s2 int "
          ") ENGINE = MEMORY "
...
void MainWindow::on_action_export_all_kompl_triggered()
{
    ...
    query.prepare(
        " SELECT "
        "sostav.id,sostav.color,sostav.status2,sostav.zakupka_status,sostav."
        "prioritet,  "
        "         sostav.notes, sostav.date, sostav.name, sostav.n1, "
        "izdelie.name, "
    ...
    str += "№\tЦвет\tСтатус\tЗакупка\tПриоритет\tМонтаж\tДата\tСпецификация\tКо"
           "л-во\tИзделие\tЗаказ\tПроект\r\n";
...
//
//==============================================================================
void MainWindow::on_action_types_triggered()
{
    TypesDialog dialog(this);
    dialog.exec();
}
...
void MainWindow::on_pb_komplSnat_clicked()
{
    ...
    if (i < 0) { tw_komplPrih->selectRow(0); }
    i = tw_komplPrih->currentRow();
    if (i < 0) return;
    ...
    {
        bool ok;
        int a1 = tw_komplKomp->item(tw_komplKomp->currentRow() - i, 2)
                     ->text()
                     .toInt(&ok);
        if (!ok) continue;
        int a2 = tw_komplKomp->item(tw_komplKomp->currentRow() - i, 3)
                     ->text()
                     .toInt(&ok);
        if (!ok) continue;
        k = a1 - a2;
    ...
    n = QInputDialog::getInt(this, app_header, tr("Количество:"), k, 1, n, 1,
                             &ok);
    if (!ok) return;
...
void MainWindow::kompl_print()
{
    ...
    if (printDialog.exec() == QDialog::Accepted)
    {
        ...
        while (1)
        {
            ...
            painter.setPen(QPen(Qt::black, 0.4, Qt::SolidLine, Qt::RoundCap,
                                Qt::RoundJoin));
            painter.drawRects(v);
            v.clear();
...
void MainWindow::dootpravka_print()
{
    ...
    query.prepare("  SELECT min(rashod.date), max(rashod.date) "
                  "  FROM rashod "
                  "    LEFT JOIN zamena ON rashod.zamena = zamena.id "
                  "    LEFT JOIN kompl  ON zamena.kompl  = kompl.id "
                  "  WHERE kompl.sostav = ? ");
...
void MainWindow::on_pb_kompl_rashod_clicked()
{
    ...
    cb_rashod_kompl->setText("КОМПЛЕКТАЦИЯ:   " + sostav + "  |  " + izdelie
                             + "\n" + zakaz + "  |  " + proekt + "  |  " + "( "
...
void MainWindow::my5()
{
    ...
    if (i) { str2 += QString(" AND type = %1 ").arg(i); }
...
{
    int i
        = cb_sklad_type_zakupki->itemData(cb_sklad_type_zakupki->currentIndex())
              .toInt();
    ...
    if (i) { str3 += QString(" AND zakupki.type = %1 ").arg(i); }

    i = cb_zakupkiFiltrPostavshik
            ->itemData(cb_zakupkiFiltrPostavshik->currentIndex())
            .toInt();
    if (i) { str3 += QString(" AND zakupki.postavshik = %1 ").arg(i); }
...
void MainWindow::timer_slot()
{
    ...
    if (komplKompl_row != i)
    {
        ...
        if (i >= 0)
        {
            ...
            tw_komplPrih->query_str_where
                = "WHERE prihod.name = \'"
                + tw_komplKomp->item(i, 0)->data(Qt::UserRole + 1).toString()
                + "\' " + str + " AND prihod.ostatok > 0";
            tw_komplPrih->refresh();
        } else
        {
            tw_komplPrih->setRowCount(0);
