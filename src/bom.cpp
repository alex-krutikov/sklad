...
QString get_positions_string(const QStringList &positions_items)
{
    ...
    if (!positions_items.length()) return "";

    rx.indexIn(positions_items[0]);
    QString prefix = rx.cap(1);

    if (prefix.isEmpty()) return "Error in postion list";
    ...
    {
        rx.indexIn(str);
        if (rx.cap(1) != prefix) return "Error in postion list";
        bool ok;
        positions << rx.cap(2).toInt(&ok);
        if (!ok) return "Error in postion list";
    }
    ...
    for (int i = 0; i < positions.size();)
    {
        ...
        {
            if (positions[j] != (positions[j - 1] + 1)) break;
        }
        --j;
...
bool BomFile::loadFile(const QString filename)
{
    ...
    if (fn[3] < 0)
    {
        QMessageBox::critical(
            0, "Импорт BOM-файла",
            QString(
                "Не найдено поле \"Value\" в первой строке BOM-файла! %1 %2")
    ...
    while (!file.atEnd())
    {
        ...
        while (k && (!file.atEnd()))
        {
            ...
            if (sl[fn[0]].count() > 0)
            {
                QMessageBox::critical(
                    0, "Импорт BOM-файла",
                    QString("Ошибка! Присутствует кол-во в подчиненной строке. "
                            "(строка %1)! ")
                        .arg(i));
                data.clear();
                return false;
...
void BomFile::process()
{
    ...
    for (i = 0; i < data.count(); i++)
    {
        if (map.contains(data[i].name))
        {
            ...
            {
                QMessageBox::critical(
                    0, "Проверка BOM файла",
                    QString("Ошибка! Найдено повторение наименования при "
                            "разных типах: \n\n"
                            "  %1 ( %2 шт.) тип:%3 \n"
                            "  %4 ( %5 шт.) тип:%6 \n")
                        .arg(d[j].name)
            ...
            //                                  .arg( d[j].name    ).arg(
            //                                  d[j].count    ).arg(
            //                                  d[j].position_items.join(", "))
            //                                  .arg( data[i].name ).arg(
            //                                  data[i].count ).arg(
            //                                  data[i].position_items.join(",
            //                                  ")));
...
bool BomAddDialog::init(QString filename, int izdelie_id, int count)
{
    ...
    query3.prepare(
        " SELECT name, sum(ostatok) FROM prihod GROUP BY type,name ");
    if (!query3.exec())
    {
    ...
    while (query3.next())
    {
        ...
        if (nalichie_map.contains(str))
        {
            QMessageBox::critical(
                0, "Импорт BOM файла",
                QString("Ошибка! В базе прихода найдено "
...
void BomAddDialog::on_tw_cellDoubleClicked(int row, int column)
{
    ...
    if (dialog.exec() == QDialog::Accepted)
    {
        ...
        if (query.next())
        {
            tw->item(row, TW_COLUMN_NALICHIE)
...
{
    if (column != TW_COLUMN_BOM_NAME) return;

    if (!(tw->item(row, TW_COLUMN_BOM_NAME)->flags() & Qt::ItemIsUserCheckable))
        return;

    bool checked
        = tw->item(row, TW_COLUMN_BOM_NAME)->checkState() == Qt::Checked;
...
void BomAddDialog::on_pb_variants_file_clicked()
{
    ...
    QString filename = QFileDialog::getOpenFileName(
        this, "Импорт VAR-файла", settings.value("varpath", "").toString(),
        "VAR (*.var)");
    ...
    for (int i = 0; i < variants.length(); ++i)
    {
        cb_variants->addItem(
            QIcon(),
            QString("%1 (%2)").arg(variants[i].name).arg(variants[i].desc));
    }
}
...
void BomAddDialog::on_cb_variants_currentIndexChanged(int index)
{
    const QSet<QString> excludes
        = (index < 0) ? QSet<QString>() : variants[index].items;
    ...
    for (int i = 0; i < data.size(); ++i)
    {
        ...
        tw->item(data2tw[i], TW_COLUMN_COUNT)
            ->setText(QString::number(data[i].count));
        tw->item(data2tw[i], TW_COLUMN_POSITION)->setText(data[i].position);
        tw->item(data2tw[i], TW_COLUMN_POSITION_EXCLUDED)
            ->setText(get_positions_string(excl_pos_list));
    }
...
void BomAddDialog::accept()
{
        ...
        if (j >= 0)
        {
            ...
            if (data[i].nominal != data[j].nominal)
            {
                QMessageBox::critical(
                    this, app_header,
                    QString(
                        "ОШИБКА! Повторение позиции с разными номиналами: %1\n")
                        .arg(str));
                return;
            }
    ...
    // подготовка: чистка таблиц. Удаляем записи в kompl и zamena, не
    // принадлежащие ни одному составу

    query.prepare(
        "DELETE zamena,kompl FROM zamena,kompl "
...
void BomAddDialog2::accept()
{
    ...
    if (!name_sklad.isEmpty())
    {
        ...
        {
            query.prepare(
                "UPDATE      bom2sklad SET name_bom = ?, name_sklad = ? ");
        } else
        {
            query.prepare(
                "INSERT INTO bom2sklad SET name_bom = ?, name_sklad = ? ");
        }
        query.addBindValue(name_bom);
...
void BomAddDialog2::on_le2_textChanged(const QString &text)
{
    ...
    if (sr.count() > 0) { str = tw->item(sr.at(0).topRow(), 0)->text(); }

    query.prepare(" SELECT name, SUM(ostatok) FROM prihod WHERE type = ? AND "
                  "name LIKE ? GROUP BY name");
