    ...
    *db = QSqlDatabase::addDatabase("QMYSQL");
    ...
    if (!query.exec("SET NAMES \'utf8\'"))
    {
        QMessageBox::critical(0, "Ошибка",
                              "Не могу установить кодировку"
                                  + query.lastError().driverText());
        done(QDialog::Rejected);
        return;
    }

    if ((le2->text() == "root") && (le3->text() == "123456"))
    {
        permissions = -1;
...
bool RVvodDialog::init_from_sostav_id(int sostav_id)
{
    ...
    if (query.size() != 1)
    {
        QMessageBox::information(
            0, "Склад",
            QString("Спецификация с номером %1 не найдена.").arg(sostav_id));
        return false;
    }
    ...
    query.prepare(
        "SELECT `name`, `nominal`, `type`, `typename`, `n`, `position` "
...
void RVvodDialog::accept()
{
    ...
    {
        query.prepare(
            "INSERT INTO kompl ( sostav, name, nominal, type, n, position ) "
            "VALUES ( ?,?,?,?,?,? ) ");
    ...
    : QDialog(parent)
{
    ...
    connect(cb_type, SIGNAL(currentIndexChanged(int)), this,
            SLOT(tw_refresh()));
    connect(le_filtr, SIGNAL(textChanged(const QString &)), this,
            SLOT(tw_refresh()));
...
void Postvshiki::on_pb1_clicked()
{
    ...
    str = QInputDialog::getText(
        this, app_header, "Название поставщика:", QLineEdit::Normal, "", &ok);
    if (!ok) return;
...
void Postvshiki::on_pb2_clicked()
{
    ...
    str = QInputDialog::getText(this, app_header,
                                "Название поставщика:", QLineEdit::Normal,
...
void Proizvoditeli::on_pb1_clicked()
{
    ...
    bool ok;
    str = QInputDialog::getText(this, app_header,
                                "Название производителя:", QLineEdit::Normal,
                                "", &ok);
    if (!ok) return;
    if (str.isEmpty()) return;
...
void SostavAddDialog::accept()
{
    ...
    case (0): // чтение BOM-файла
    {
        ...
        QString str = QFileDialog::getOpenFileName(
            this, "Импорт BOM-файла", settings.value("bompath", "").toString(),
            "BOM (*.bom)");
        if (!str.isEmpty())
        {
            BomAddDialog dialog(this);
            if (!dialog.init(str, id, sb1->value()))
            {
                done(QDialog::Rejected);
                return;
            }
    ...
    case (1): // ручной ввод
    {
        ...
        if (dialog.result()) done(QDialog::Accepted);
    }
    break;
    case (2): // повтор спецификации по номеру
    {
        ...
        {
            rv_dialog.exec();
            if (rv_dialog.result()) done(QDialog::Accepted);
        }
    }
    break;
    }
    ...
    : QDialog(parent)
{
    ...
    QSqlQuery query;
    // тип
    if (!query.exec("SELECT id, typename FROM types WHERE used = TRUE ORDER BY "
                    "typename"))
    {
        sql_error_message(query, this);
        return;
    ...
    , zakupka(zakupka_arg)
{
    ...
    setupUi(this);
    if (id)
        setWindowTitle("Изменить данные в приходе");
    else
        setWindowTitle("Добавить приход");
    ...
    while (query.next())
    {
        ...
        if (map.contains(str) && (map[str] != a))
        {
            QMessageBox::critical(
                this, app_header,
                "Ошибка! База прихода содержит несколько одинаковых "
                "наименований с разными типами!\n\n"
                    + str);
        }
        map[str] = a;
    }
    ...
    sl.clear();
    if (!query.exec("SELECT naklad FROM prihod GROUP BY naklad ORDER BY "
                    "MAX(id) DESC LIMIT 10"))
    {
        sql_error_message(query, this);
        return;
    }
    ...
    while (query.next())
    {
        ...
        // QMessageBox::critical(this, QObject::tr("Ошибка запроса SQL"),
        // QString::number(cb1->findData( query.value(0) ) ) );
        le1->clear();
...
void PrihodAddDialog::accept()
{
    ...
    if (zakupka || (id == 0)) { le4->setText(le6->text()); }
    ...
    {
        query.prepare(
            "UPDATE zakupki SET polucheno = polucheno + ? WHERE id = ?");
        query.addBindValue(le6->text().toInt()); // количество
        query.addBindValue(zakupka);
        if (!query.exec())
        {
    ...
    , id(id_arg)
{
    ...
    tw->query_str_main
        = " FROM ( SELECT id,type,name,ostatok FROM prihod "
...
void ZamenaAddDialog::on_pb_rename_clicked()
{
    ...
    if (query.size() == 1)
    {
        ...
        if (query.value(1) == 0)
        {
            ...
            query.prepare(
                " SELECT id FROM zamena WHERE kompl = ? AND name = ? ");
            query.addBindValue(id);
            query.addBindValue(str);
            if (!query.exec())
            {
    ...
    QMessageBox::critical(this, app_header,
                          QString("Переименовать невозможно:\n"
                                  "ОШИБКА БАЗЫ. [KOMPL_ID = %1]")
                              .arg(id));
}
    ...
    , id(id_arg)
{
    ...
    setupUi(this);
    if (id)
        setWindowTitle("Изменить данные в в закупке");
    else
        setWindowTitle("Добавить закупку");
    ...
    connect(le1, SIGNAL(textChanged(QString)), this, SLOT(my1(QString)));
    //-------------------------------------------------------------------------
    cb5->clear();
    if (!query.exec(
            "SELECT schet FROM zakupki GROUP BY schet ORDER BY max(id) DESC"))
    {
        sql_error_message(query, this);
        return;
    }
    ...
    cb6->clear();
    if (!query.exec(
            "SELECT platej FROM zakupki GROUP BY platej ORDER BY MAX(id) DESC"))
    {
        sql_error_message(query, this);
        return;
    }
    ...
    : QDialog(parent)
{
    ...
    tw->query_str_main = " FROM users ";
    tw->query_str_order_by = " ORDER BY username ASC ";
...
void UsersDialog::on_pb2_clicked()
{
    QSqlQuery query;
    query.prepare(
        " INSERT INTO users ( username, nickname, passw, permissions ) "
...
void UsersDialog::my1()
{
    int i;
    QSqlQuery query;
    query.prepare("SELECT nickname, username, passw, permissions FROM users "
                  "WHERE id = ? ");
    query.addBindValue(tw->get_selected_id());
    if (!query.exec())
    {
