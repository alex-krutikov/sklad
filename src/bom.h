...
class BomFile
{
    ...
    struct t_data
    {
        ...
        bool operator<(const t_data &t1) const
        {
            return (type < t1.type)
                     ? (true)
                     : ((type == t1.type) ? (name < t1.name) : (false));
        }
    };
    ...
    , public Ui::BomAddDialog
{
    ...
    struct t_data
    {
        ...
        bool operator<(const t_data &t1) const
        {
            return (type_name < t1.type_name)
                     ? (true)
                     : ((type_name == t1.type_name) ? (name_bom < t1.name_bom)
                                                    : (false));
        }
    };
...
public:
    ...
    bool init(QString filename = QString(), int izdelie_id = 0, int count = 1);
