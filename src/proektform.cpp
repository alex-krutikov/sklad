...
void ProektForm::on_tw_proekt_itemSelectionChanged()
{
  ...
  if( sr.count() )
  { current_proekt_id = tw_proekt->item( sr.at(0).topRow(), 0 )->data(Qt::UserRole).toInt();
  } else
  { current_proekt_id = -1;
  }
  zakaz_refresh();
...
void ProektForm::on_tw_zakaz_itemSelectionChanged()
{
  ...
  if( sr.count() )
  { current_zakaz_id = tw_zakaz->item( sr.at(0).topRow(), 0 )->data(Qt::UserRole).toInt();
  } else
  { current_zakaz_id = -1;
  }
  izdelie_refresh();
...
void ProektForm::on_tw_izdelie_itemSelectionChanged()
{
  ...
  if( sr.count() )
  { current_izdelie_id = tw_izdelie->item( sr.at(0).topRow(), 0 )->data(Qt::UserRole).toInt();
  } else
  { current_izdelie_id = -1;
  }
  sostav_refresh();
...
{ int i;
  ...
  query.prepare( " SELECT id,name,n1,prioritet,status2,date,color,notes, cost_kompl, cost_plata, cost_montag "
...
void ProektForm::on_tw_sostav_itemSelectionChanged()
{
  ...
  if( sr.count() )
  { current_sostav_id = tw_sostav->item( sr.at(0).topRow(), 0 )->data(Qt::UserRole).toInt();
  } else
  { current_sostav_id = -1;
  }
  if( current_sostav_id > 0 ) { le_sostav->setText( QString::number( current_sostav_id ) ); }
  else                        { le_sostav->setText("");                                     }
  kompl_refresh();
}
...
void ProektForm::on_tw_sostav_cellDoubleClicked( int row, int column )
{
  ...
  switch( column )
  {
    ...
    case( 2 ):  // количество
      ...
      i = QInputDialog::getInt(this, app_header, "Количество:",
                                      i,1,1000,1, &ok);
      if( !ok ) return;
...
void ProektForm::on_pb_sostav_export_to_file_clicked()
{
  ...
  { for(i=0; i<nc; i++ )
    {  QTableWidgetItem *titem = tw_sostav->item(j,i);
       if( titem )  str += titem->text().simplified() + ";";
       else         str += ";";
    }
    str.chop(1);
    str += "\n";
...
void ProektFormSostavFindWidget::refresh()
{
  ...
  query.prepare( "SELECT id,name FROM sostav WHERE name LIKE ? ORDER BY name ASC" );
  ...
  if( sr.count() ) a=tw->item( sr.at(0).topRow(), 0 )->data(Qt::UserRole).toInt();
  tw->clearSelection();
  tw->clearContents();
...
void ProektFormSostavFindWidget::on_tw_itemSelectionChanged()
{
  int id=0;
  QList<QTableWidgetSelectionRange> sr = tw->selectedRanges();
  if( sr.count() ) id=tw->item( sr.at(0).topRow(), 0 )->data(Qt::UserRole).toInt();
  if( id == 0 ) return;
